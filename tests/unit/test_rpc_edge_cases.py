...
from rpc_helper.rpc import RpcHelper, get_contract_abi_dict, get_encoded_function_signature, get_event_sig_and_abi
...
class TestRpcEdgeCases:
    ...
    async def test_zero_address_balance(self, rpc_helper_instance):
        ...
        result = await rpc_helper_instance.batch_eth_get_balance_on_block_range(zero_address, 12345678, 12345678)
    ...
    async def test_network_timeout_handling(self, rpc_helper_instance):
        ...
        with pytest.raises(RPCException) as exc_info:
            await rpc_helper_instance.batch_eth_get_balance_on_block_range(address, 12345678, 12345678)
    ...
    async def test_memory_exhaustion_prevention(self, rpc_helper_instance):
        ...
        result = await rpc_helper_instance.batch_eth_get_balance_on_block_range(address, 12345678, 12345777)
    ...
    async def test_missing_trie_node_error_handling(self, rpc_helper_instance):
        ...
        result = await rpc_helper_instance.batch_eth_get_balance_on_block_range(address, 12345678, 12345678)
    ...
    async def test_invalid_abi_handling(self, rpc_helper_instance):
        ...
        invalid_abi = [{"name": "invalidFunction", "type": "function", "inputs": "invalid"}]
    ...
    async def test_unicode_and_special_characters_in_data(self, rpc_helper_instance):
        ...
        mock_response = rpc_helper_instance._client.post.return_value
        mock_response.status_code = 200
        mock_response.set_json_data([{"result": "0x" + "ff" * 32}])  # All bytes set to 0xFF
